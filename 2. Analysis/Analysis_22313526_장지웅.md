...
> 최종 구현 반영 메모: Analysis 단계에서는 GPU 기반 판별 엔진을 주요 구현 방향으로 고려하였다. 최종 Implementation 단계에서는 웹 링크 제출과 서버 상시 운영 조건을 고려하여 Render 기반 Python 웹 서비스와 Sightengine AI Image Detection API를 연동하는 방식으로 구현하였다. 히트맵은 API의 직접적인 attention map이 아니라, 이미지 패치별 경계 변화, 고주파 질감, 색상 분산, 압축 블록 경계 지표를 계산하여 전체 AI 생성 확률을 보조 설명하는 시각 자료로 제공한다.
